...
export function CustomerActivity({ statusHistory, emailHistory }: { statusHistory: TimelineEntry[]; emailHistory: EmailLogEntry[] }) {
  ...
  const fromStatus: ActivityItem[] = statusHistory
    ...
    .map((e) => ({
      at: e.changedAt,
      icon: e.toStatus === 'accepted' ? <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400" /> : e.toStatus === 'declined' ? <XCircle className="h-4 w-4 text-red-600 dark:text-red-400" /> : e.toStatus === 'expired' ? <Clock className="h-4 w-4 text-orange-600" /> : <Eye className="h-4 w-4 text-purple-600" />,
  ...
  if (items.length === 0) {
    return <p className="text-sm text-slate-400 dark:text-slate-500">No customer activity yet.</p>;
  }
      ...
      {items.map((item, i) => (
        <li key={i} className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">{new Date(item.at).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}</p>
          </div>
        </li>
      ))}
    </ul>
