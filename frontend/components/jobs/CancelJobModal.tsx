    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={isSubmitting ? undefined : onClose}>
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Cancel Job?</h2>
        ...
        <div className="mt-4 space-y-1 rounded-lg bg-slate-50 p-3 text-sm">
          <p><span className="text-slate-500">Customer: </span><span className="font-medium text-slate-800">{customerName}</span></p>
          <p><span className="text-slate-500">Property: </span><span className="font-medium text-slate-800">{propertyAddress}</span></p>
          {scheduledStart && (
            <p><span className="text-slate-500">Scheduled: </span><span className="font-medium text-slate-800">{new Date(scheduledStart).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}</span></p>
          )}
          {jobTotal !== null && (
            <p><span className="text-slate-500">Job Total: </span><span className="font-medium text-slate-800">${jobTotal.toFixed(2)}</span></p>
          )}
        </div>

        <p className="mt-3 text-xs text-slate-500">This changes the job's status to Cancelled. There's no direct "undo" button for this in the app today.</p>

        <label className="mt-4 block text-sm font-medium text-slate-700">Cancellation reason</label>
        <select
          value={reasonOption}
        ...
        {reasonOption === 'Other' && (
          <input
            ...
            maxLength={500}
            className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:bg-slate-50 disabled:opacity-60"
          />
        )}
