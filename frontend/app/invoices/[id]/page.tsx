  ...
  return (
    <AppShell>
      <main className="mx-auto max-w-3xl px-4 py-6 sm:px-6 lg:py-8">
        <Link href="/invoices" className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800">← Back to Invoices</Link>

        {isLoading && <div className="mt-6 text-sm text-slate-500 dark:text-slate-400">Loading…</div>}
        {error && <div className="mt-6 text-sm text-red-600 dark:text-red-400">Couldn't load this invoice.</div>}
          ...
          <>
            <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{invoiceCustomerName(invoice)} · {invoice.propertyAddressLine1}, {invoice.propertyCity}</p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              {!['paid', 'void', 'partial'].includes(invoice.status) && (
                <button onClick={handleVoid} disabled={isActing} className="rounded-lg border border-slate-300 dark:border-slate-700 px-5 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 disabled:opacity-50">
                  Void
                </button>
              )}
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:grid-cols-4">
              <div>
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Issue Date</p>
                <p className="mt-0.5 text-sm text-slate-900 dark:text-slate-100">{formatDate(invoice.createdAt)}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Due Date</p>
                <p className="mt-0.5 text-sm text-slate-900 dark:text-slate-100">{formatDate(invoice.dueDate)}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Sent</p>
                <p className="mt-0.5 text-sm text-slate-900 dark:text-slate-100">{invoice.sentAt ? formatDate(invoice.sentAt) : 'Not sent'}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Paid</p>
                <p className="mt-0.5 text-sm text-slate-900 dark:text-slate-100">{invoice.paidAt ? formatDate(invoice.paidAt) : 'Not paid'}</p>
              </div>
            </div>
            ...
            <div className="mt-6 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
              ...
              <div className="space-y-1 border-t border-slate-200 dark:border-slate-800 px-4 py-3 text-sm">
                <div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Subtotal</span><span>{formatMoney(invoice.subtotal)}</span></div>
                {Number(invoice.discountAmount) > 0 && <div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Discount</span><span>-{formatMoney(invoice.discountAmount)}</span></div>}
                <div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Tax ({(Number(invoice.taxRate) * 100).toFixed(2)}%)</span><span>{formatMoney(invoice.taxAmount)}</span></div>
                <div className="flex justify-between text-base font-semibold text-slate-900 dark:text-slate-100"><span>Total</span><span>{formatMoney(invoice.totalAmount)}</span></div>
                <div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Amount Paid</span><span>{formatMoney(invoice.amountPaid)}</span></div>
                <div className="flex justify-between text-base font-semibold text-slate-900 dark:text-slate-100"><span>Balance Due</span><span>{formatMoney(invoice.balanceDue)}</span></div>
              </div>
            </div>
              ...
              <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                {invoice.notes && (
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{invoice.notes}</p>
                  </div>
                )}
                {invoice.terms && (
                  <div>
                    <h2 className="text-sm font-medium text-slate-700 dark:text-slate-300">Terms</h2>
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{invoice.terms}</p>
                  </div>
                )}
              </div>
            )}
