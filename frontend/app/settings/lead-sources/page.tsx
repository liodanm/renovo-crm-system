        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="space-y-2">
            {options.map((option, index) => (
              <div key={option.key} className="flex items-center gap-2 rounded-lg border border-slate-100 p-2">
                <div className="flex flex-col">
                  <button type="button" onClick={() => moveItem(index, -1)} disabled={index === 0} className="flex h-6 w-6 items-center justify-center text-slate-400 disabled:opacity-25">
                    <ChevronUp className="h-3.5 w-3.5" />
                  </button>
                  <button type="button" onClick={() => moveItem(index, 1)} disabled={index === options.length - 1} className="flex h-6 w-6 items-center justify-center text-slate-400 disabled:opacity-25">
                ...
                <input
                  ...
                  className="flex-1 rounded-lg border border-slate-300 px-3 py-3 text-base lg:py-1.5 lg:text-sm"
                />
                <label className="flex shrink-0 items-center gap-1.5 text-xs text-slate-500">
                  <input type="checkbox" checked={option.enabled} onChange={() => toggleEnabled(option.key)} className="h-4 w-4 rounded border-slate-300" />
                  Enabled
                </label>
                <button type="button" onClick={() => remove(option.key)} aria-label={`Remove ${option.label}`} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600">
                  <X className="h-4 w-4" />
                </button>
              </div>
            ))}
