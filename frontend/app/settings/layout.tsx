        ...
        <aside className="shrink-0 lg:w-64">
          <div className="mb-3">
            ...
            <nav aria-label="Breadcrumb" className="mt-1 flex items-center gap-1 text-xs text-slate-400">
              <Link href="/settings/profile" className="hover:text-slate-600 dark:hover:text-slate-300">Settings</Link>
              {activeItem && (
                <>
                  <ChevronRight className="h-3 w-3" />
                  <span className="text-slate-600 dark:text-slate-300">{activeItem.label}</span>
                </>
              )}
            </nav>
          </div>
          ...
          <div className="relative mb-3">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-300 py-3 pl-8 pr-3 text-base lg:py-2 lg:text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500"
            />
          </div>
                      ...
                      <li key={item.key}>
                        <Link
                          ...
                          className={cn(
                            ...
                            isActive ? 'bg-[var(--color-brand)]/10 font-medium text-[var(--color-brand)]' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800',
                          )}
                        >
                          {item.label}
                          {item.comingSoon && <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-400 dark:bg-slate-800">Soon</span>}
                        </Link>
                      </li>
                    );
                  })}
