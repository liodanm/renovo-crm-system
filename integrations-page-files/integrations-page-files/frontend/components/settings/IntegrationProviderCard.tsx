    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">{card.feature}</p>
          </div>
        </div>
        <span className={`flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${card.configured ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
      ...
      {!card.configured && (
        <p className="mt-3 text-xs text-slate-500">
          Set these in your Railway environment variables: <code className="rounded bg-slate-100 px-1 py-0.5">{card.missingVars.join(', ')}</code>
        </p>
      )}
      ...
      <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
        ...
        <div className="contents">
          <dt className="text-slate-400">Last Verification</dt>
          <dd className="text-right text-slate-600">{card.lastVerifiedAt ? `${timeAgo(card.lastVerifiedAt)}${card.verifyOk === false ? ' (failed)' : ''}` : 'never'}</dd>
        </div>
        <div className="contents">
          <dt className="text-slate-400">Last Test</dt>
          <dd className="text-right text-slate-600">{card.lastTestAt ? `${timeAgo(card.lastTestAt)}${card.testOk === false ? ' (failed)' : ''}` : 'never'}</dd>
        </div>
        {(card.verifyError || card.testError) && (
          <div className="contents">
            <dt className="text-slate-400">Last Error</dt>
            <dd className="text-right text-red-600">{card.verifyError ?? card.testError}</dd>
          </div>
        )}
      </dl>

      {message && <p className={`mt-3 text-xs ${message.type === 'error' ? 'text-red-600' : 'text-emerald-600'}`}>{message.text}</p>}

      {testInput && <div className="mt-3">{testInput}</div>}

      <div className="mt-3 flex flex-wrap items-center gap-2">
        <button onClick={handleVerify} disabled={isVerifying || !card.configured} className="flex items-center gap-1.5 rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50">
        ...
        <a href={docsUrl} target="_blank" rel="noreferrer" className="ml-auto flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600">
