...
export class StripePaymentService {
  ...
  /**
   * Confirms STRIPE_SECRET_KEY is valid and reports whether it's a
   * live or test key — read from Stripe's own response (`livemode`),
   * not guessed from the key's `sk_live_`/`sk_test_` prefix. Used by
   * the Integrations page's "Verify Connection" for Stripe; does not
   * touch STRIPE_WEBHOOK_SECRET (see verifyWebhookSecretPresence below).
   */
  async verifyConnection(): Promise<{ ok: boolean; error?: string; livemode?: boolean }> {
    const secretKey = this.config.get<string>('STRIPE_SECRET_KEY');
    ...
    try {
      ...
      if (!response.ok) {
        return { ok: false, error: `stripe_error_${response.status}` };
      }
      const data = await response.json();
      return { ok: true, livemode: !!data.livemode };
    } catch (err) {
